...
Runs the full Plexus stack on your machine: the **frontend natively** (`npm run dev`, hot reload) and **everything else in Docker**, built from this monorepo's service directories. One command.
...
**Requires:** Docker. The backend services (`gateway`, `houston`, `api`, `clickhouse`) are built straight from this repo's top-level directories — nothing extra to clone. The `recorder` service is internal-only (a private repo, not part of this monorepo) and sits behind its own compose profile (`--profile recorder`).
...
`data-api` is **off by default** (compose `extras` profile) — turn it on with `EXTRAS=1`. `recorder` sits behind its own `recorder` profile and is internal-only (needs the private `video-recorder` repo cloned to the repo root first).
...
- **Build-from-source.** Images are built from this repo's service directories (`../../<service>`). First build compiles 4 images (5 with the `recorder` profile; ~a few minutes); after that they're cached.
