...
const sheetVariants: Record<string, string> = {
  right:
    "inset-y-0 right-0 h-full w-3/4 sm:max-w-md border-l data-[state=open]:slide-in-from-right data-[state=closed]:slide-out-to-right",
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed z-50 gap-4 bg-card p-6 shadow-lg duration-300 ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out",
        sheetVariants[side],
        className
      )}
      {...props}
