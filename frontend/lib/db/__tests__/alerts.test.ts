...
describe("alertQueries.getOpenCountsBySource", () => {
  async function insertAlertFor(
    sourceId: string,
    ...
    await db.insert(alerts).values({
      ...
      severity: "info",
      status,
      is_alert_active: status === "open",
    } as typeof alerts.$inferInsert);
  }

  it("counts only open alerts, grouped by source", async () => {
    const a = await insertSource();
    const b = await insertSource();
    await insertAlertFor(a, "open");
    await insertAlertFor(a, "open");
    await insertAlertFor(a, "resolved"); // not open — excluded
    await insertAlertFor(b, "open");

    const counts = await alertQueries.getOpenCountsBySource(ORG);
    const map = new Map(counts.map((c) => [c.source_id, c.count]));
    expect(map.get(a)).toBe(2);
    expect(map.get(b)).toBe(1);
  });

  it("respects the source allow-list scope", async () => {
    const a = await insertSource();
    const b = await insertSource();
    await insertAlertFor(a, "open");
    await insertAlertFor(b, "open");

    const counts = await alertQueries.getOpenCountsBySource(ORG, [a]);
    const map = new Map(counts.map((c) => [c.source_id, c.count]));
...
describe("alertQueries.getNoiseBySource", () => {
  async function insertVerdicted(
    sourceId: string,
    verdict: "helpful" | "noise" | null,
  ): Promise<void> {
    await db.insert(alerts).values({
  ...
  it("tallies noise vs total verdicted per source", async () => {
    const a = await insertSource();
    await insertVerdicted(a, "noise");
    await insertVerdicted(a, "noise");
    await insertVerdicted(a, "helpful");
    await insertVerdicted(a, null); // unverdicted — excluded from total

    const rows = await alertQueries.getNoiseBySource(ORG);
    const row = rows.find((r) => r.source_id === a);
    expect(row).toEqual({ source_id: a, noise: 2, total: 3 });
  });
});
